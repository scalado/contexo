...
def cmd_info(args):
    ...
    if args.module != None:
        depmgr = CTXDepMgr ( codeModulePaths = cview.getItemPaths('modules'), tolerateMissingHeaders = args.tolerate_missing_headers, archPath = bc.getArchPath(), legacyCompilingMod = args.legacy_compiling_mod )
...
def cmd_buildcomp(args):
    ...
    bc          = getBuildConfiguration( cview,  args )
    # depmgr      = CTXDepMgr ( cview.getItemPaths('modules') ,  args.tolerate_missing_headers, bc.getArchPath(), args.legacyBuildMod )
    depmgr = CTXDepMgr ( codeModulePaths = cview.getItemPaths('modules'), tolerateMissingHeaders = args.tolerate_missing_headers, archPath = bc.getArchPath(), legacyCompilingMod = args.legacy_compiling_mod )
    session     = ctx_base.CTXBuildSession( bc )
    session.setDependencyManager( depmgr )
